...
-- 1. Index Barcode (Sudah ada UNIQUE, tapi pastikan ini yang utama)
-- Barcode adalah kolom yang paling sering dipukul oleh Next.js
CREATE INDEX idx_products_barcode ON products(barcode);

-- 2. Index untuk Pencarian Produk berdasarkan Nama (Autocomplete)
CREATE INDEX idx_products_name ON products(product_name);

-- 3. Composite Index untuk Penjualan (Sangat Penting untuk Laporan)
-- Mempercepat filter penjualan berdasarkan tanggal dan status pembayaran
CREATE INDEX idx_sales_created_status ON sales(created_at, payment_status);

-- 4. Index FIFO/FEFO pada Stock Batches
-- Mempercepat query: "Cari batch produk X yang belum habis dan paling cepat expired"
CREATE INDEX idx_batches_fifo_fefo ON stock_batches(product_id, remaining_qty, expiry_date);

-- 5. Index Invoice Number
